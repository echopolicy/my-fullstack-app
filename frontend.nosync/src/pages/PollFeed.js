...
const PollFeed = () => {
  ...
  const handleCopyLink = (url) => {
    ...
    navigator.clipboard.writeText(url).then(() => {
      ...
      setTimeout(() => setShowCopyToast(false), 2500); // Hide toast after 2.5 seconds
    }).catch(err => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-6">All Active Polls</h2>
      ...
      {polls.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {polls.map(poll => {
            // Define the URL and title for sharing
